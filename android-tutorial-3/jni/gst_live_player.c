...
// We don't need that verbose ui
static void
set_ui_message (const gchar * message, CustomData * data)
{
  __android_log_print (ANDROID_LOG_DEBUG, "tutorial-3",
                       "Was ui message %s",message);
}
...
app_function (void *userdata)
{
  ...
  /* Build pipeline */
  data->pipeline =
      gst_parse_launch ("videotestsrc ! autovideosink",
      &error);
  /*data->pipeline =
          gst_parse_launch ("udpsrc port=5600 caps = \"application/x-rtp, media=(string)video, encoding-name=(string)H264, payload=(int)96\" ! rtph264depay ! h264parse ! decodebin ! autovideosink sync=false",
                            &error);*/
  if (error) {
    gchar *message =
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
  ...
  jclass klass = (*env)->FindClass (env,
      "org/freedesktop/gstreamer/tutorials/tutorial_3/GstLivePlayer");
